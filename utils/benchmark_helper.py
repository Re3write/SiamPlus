...
def load_dataset(dataset):
    ...
    if 'VOT' in dataset:
        ...
        # base_path = join('/home/sk49/workspace/cy/Siamplus/data','DAVIS'+dataset[-4:])
        list_path = join(realpath(dirname(__file__)), '../data', 'DAVIS', 'ImageSets', dataset[-4:], 'val.txt')
        # list_path = join('/home/sk49/workspace/cy/Siamplus/data', 'DAVIS'+dataset[-4:], 'ImageSets/2016', 'val.txt')
        with open(list_path) as f:
            videos = [v.strip() for v in f.readlines()]
        for video in videos:
            print(video)
